...
<img src="readme_md_img/stripe_img/stripe4.png" alt="Stripe ダッシュボード" width="600">
...
**苦労した点**
  ...
  - 予約時の日付選択で、カレンダー用の Select には placeholder が使えず、デフォルトで「今日の日付」を表示する実装にしたところ、**「未選択」状態にならないためバリデーションが働かず、そのまま予約できてしまう**問題に直面しました。「カレンダーを開いて日付をクリックしないとバリデーションエラーになる」という一般的な挙動に揃えるまで、試行錯誤して苦労しました。

  - 開発中にデベロッパーツールで確認していたところ、リロードするとメイン画面が表示されなくなる事象が発生しました。原因は **main 要素に高さだけ指定して幅を指定していなかった** ことでした。単純な CSS の指定漏れでしたが、原因の特定に数時間を要しました。

**学んだこと**

  - 上記の苦労を通じて、**経験が少ないほど単純なミスが盲点になりやすい**と学びました。不具合が起きたときは、まず単純なところ（CSS の指定漏れや入力値など）から見直すように心がけています。
